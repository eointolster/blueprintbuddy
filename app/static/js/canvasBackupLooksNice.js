class BlueprintCanvas {
    ...
    constructor() {
        ...
        // Make double-click for new components only work on the canvas background
    ...
    renderComponent(component) {
        ...
        // Component name with explicit event handling
        const titleText = g.append('text')
            ...
            .attr('fill', '#90caf9')
            .attr('font-size', '14px')
            .style('pointer-events', 'all'); // Make sure text can receive events
    ...
    editComponentName(g, component) {
        const textElement = g.select('.component-title');
        ...
        const input = fo.append('xhtml:input')
            ...
            .style('width', '100%')
            .style('background', 'rgba(13, 71, 161, 0.9)')
        ...
        const finishEditing = () => {
            ...
            textElement.text(newName).style('opacity', '1');
            
            // Use timeout to ensure we're outside the current event loop
            setTimeout(() => {
                if (fo.node() && fo.node().parentNode) {
                    fo.remove();
                }
            }, 0);
        };
